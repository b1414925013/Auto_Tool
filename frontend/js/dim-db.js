// DIM数据库管理页面脚本
import { DimDBApi, Utils } from './api.js';
...
async function loadPasswords(page = 1, limit = 10) {
    ...
    try {
        ...
        const data = await DimDBApi.getAll(skip, limit);
        const totalCount = await DimDBApi.getTotalCount();
...
async function editPassword(id) {
    try {
        const password = await DimDBApi.getById(id);
        ...
        $('#password-modal-label').text('编辑DIM数据库密码');
        $('#password-modal').modal('show');
    } catch (error) {
...
async function savePassword() {
    ...
    try {
        if (id) {
            await DimDBApi.update(id, data);
            Utils.showAlert('密码更新成功', 'success');
        } else {
            await DimDBApi.create(data);
