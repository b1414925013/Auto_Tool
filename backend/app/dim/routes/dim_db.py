...
# 获取所有DIM数据库密码
@dim_db_router.get("/dim-db", response_model=Dict[str, Any])
async def get_dim_db(
    skip: int = 0,
    limit: int = 100,
):
    total = await DimDBModel.all().count()
...
async def get_dim_db(
    ...
    # 将模型实例转换为 Pydantic 模型实例
    return DimDB.model_validate(password)
...
async def create_dim_db(
    ...
    # 将模型实例转换为 Pydantic 模型实例
    return DimDB.model_validate(db_password)
